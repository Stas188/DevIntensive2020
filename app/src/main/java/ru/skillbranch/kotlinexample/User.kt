...
class User private constructor(
    ...
    private var phone: String? = null
        set(value) {
            ...
            else field = value.trimPhoneNumber()//?.replace("[^+\\d]".toRegex(), replacement = "")
        }
    ...
    fun changePassword(number: String){
        val code = generateAccessCode()
        passwordHash = encrypt(code)
        accessCode = code
        sendAccessCodeToUser(number,code)
    }
...
/// inesty v string
fun String.isValidNumber() = this.trimPhoneNumber().matches(Regex("""^\+\d{11}"""))
fun String.trimPhoneNumber() = this.replace("[^+\\d]".toRegex(), replacement = "")